...
"""
Fix empty LVM backend network values in a System manifest.
...
Usage:
    fix_lvm_backend_network.py input.yaml > output.yaml
...
def fix_system_lvm_network(doc):
    ...
    if changes:
        print(
            f"Updated network for {changes} lvm backend(s) in System '{name}'",
            file=sys.stderr
        )
...
def main():
    ...
    cleaned = [fix_system_lvm_network(d) for d in docs if d is not None]
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: fix_lvm_backend_network.py input.yaml > output.yaml", file=sys.stderr)
        sys.exit(1)
    main()
